...
def main() -> None:
    ...
    store=UserStore()
    autopilot = Autopilot(service, state_store=store)
    ...
    crypto=CryptoAutopilot(broker, state_store=store); crypto.set_assets(crypto_data.crypto_assets())
    crypto_runner=CryptoAutopilotRunner(crypto, crypto_data)
    controller=TelegramPaperController(service, settings.allowed_user_ids, autopilot=autopilot, autopilot_runner=runner, crypto_autopilot=crypto, crypto_runner=crypto_runner, store=store, market_data=market_data)
